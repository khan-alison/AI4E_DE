...
SELECT c.cust_id,
       c.cust_nm,
       TO_CHAR(ct.transaction_time, 'mm-yy') as transaction_month,
...
WITH DebitTransactions AS (SELECT c.cust_id,
                                  cus.cust_nm                                            AS customer_name,
                           ...
                           FROM ai4e_test.card_transaction ct
                                ...
                                ai4e_test.card c ON ct.card_id = c.card_id
                                    JOIN
                                ai4e_test.customer cus ON c.cust_id = cus.cust_id
                                    JOIN
                                ai4e_test.card_type ct_type ON c.card_type = ct_type.type_id
                           WHERE ct_type.type_nm = 'DEBIT'
                           GROUP BY c.cust_id, cus.cust_nm)
...
SELECT c.cust_id,
       TO_CHAR(transaction_time, 'mm-yy')                     as transaction_month,
       SUM(CASE WHEN pt.amount < 0 THEN pt.amount ELSE 0 END) as total_expenditure
FROM ai4e_test.payment_transaction pt
         JOIN
...
SELECT dt.cust_id,
       ...
       COALESCE(LAG(dt.total_expenditure) OVER (PARTITION BY dt.cust_id ORDER BY dt.transaction_month),
                0)          AS previous_month_expenditure,
       dt.total_expenditure AS current_month_expenditure
FROM DebitTransactions dt
ORDER BY dt.cust_id,
         dt.transaction_month;
...
SELECT dt.cust_id,
       ...
       SUM(dt.monthly_expenditure)
       OVER (PARTITION BY dt.cust_id ORDER BY dt.transaction_month) AS cumulative_expenditure
FROM DebitTransactions dt
ORDER BY dt.cust_id,
         dt.transaction_month;
